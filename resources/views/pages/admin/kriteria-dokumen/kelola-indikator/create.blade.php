...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="#">Kelola Indikator</a></li>
    <li class="breadcrumb-item active" aria-current="page">Tambah Indikator</li>
  </ol>
</nav>
{{-- @dd($importTitle, $element, $allElements) --}}
<div class="row">
  <div class="col-lg-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Tambah Indikator</h4>
        <form action="{{ route('admin.kriteria-dokumen.kelola-indikator.store') }}" method="POST" enctype="multipart/form-data" id="KelolaTargetForm">
          @csrf
					<div class="mb-4">
						<label for="standardSelector" class="form-label">Pilih Dimensi:</label>
						<select id="standardSelector" class="form-select">
							<option value="element-{{ $element->id }}" selected>{{ $element->nama }} (Aktif)</option>
							@foreach($allElements->where('id', '!=', $element->id) as $item)
								<option value="element-{{ $item->id }}">{{ $item->nama }}</option>
							@endforeach
						</select>
					</div>
					<div id="buktiContainer">
						@foreach($allElements as $item)
              <div id="element-{{ $item->id }}" class="bukti-list" style="{{ $item->id === $element->id ? '' : 'display: none;' }}">
                <h6 class="fw-bold">{{ $item->nama }}</h6>
                <ul>
                  @forelse($item->indicators as $indikator)
                      <li>{{ $indikator->nama_indikator }} @if($indikator->info) - {{ $indikator->info }} @endif</li>
                  @empty
                      <li><em>Belum ada indikator</em></li>
                  @endforelse
                </ul>
              </div>
            @endforeach
          ...
          <div class="row">
            <div class="col-sm-6">
              <div class="mb-3">
                ...
                <input id="nama" class="form-control @error('nama') is-invalid @enderror" name="nama" type="text" value="{{ old('nama') }}" placeholder="Nama Indikator">
                @error('nama')
          ...
          {{-- @dd($element) --}}
					<input type="hidden" name="elemen_id" id="elemen_id" value="{{ $element->id }}">
...
@push('custom-scripts')
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    selector.addEventListener('change', function () {
			...
			const selectedId = this.value.replace('element-', '');
			document.getElementById('elemen_id').value = selectedId;
		});
